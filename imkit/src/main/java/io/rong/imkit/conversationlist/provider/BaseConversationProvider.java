...
import io.rong.imlib.model.DataManagementInfo;
...
public class BaseConversationProvider implements IViewProvider<BaseUiConversation> {
    ...
    public void bindViewHolder(
        ...
        // 会话标题：优先使用托管信息中的名称
        holder.setText(R.id.rc_conversation_title, getConversationTitle(uiConversation.mCore));

        // 会话头像：优先使用托管信息中的头像
        String portraitUrl = getConversationPortraitUrl(uiConversation.mCore);
        ...
        if (isDebugMode(holder.getContext())) {
            String debugTitle = getConversationTitle(uiConversation.mCore);
            if (uiConversation.mCore.getConversationType()
                ...
                holder.setText(
                        R.id.rc_conversation_title,
                        debugTitle + "(" + uiConversation.mCore.getPushNotificationLevel() + ")");
            }
        }
    ...
    private String getConversationTitle(Conversation conversation) {
        // 优先使用托管信息中的名称
        DataManagementInfo dataManagementInfo = conversation.getDataMgmtInfo();
        if (dataManagementInfo != null && !TextUtils.isEmpty(dataManagementInfo.getName())) {
            return dataManagementInfo.getName();
        }
        // 否则使用原有逻辑
    ...
    /**
     * 获取会话头像 URL，优先使用托管信息中的头像
     *
     * @param conversation 会话对象
     * @return 会话头像 URL
     * @since 5.36.0
     */
    private String getConversationPortraitUrl(Conversation conversation) {
        // 优先使用托管信息中的头像
        DataManagementInfo dataManagementInfo = conversation.getDataMgmtInfo();
        if (dataManagementInfo != null && !TextUtils.isEmpty(dataManagementInfo.getPortraitUri())) {
            return dataManagementInfo.getPortraitUri();
        }
        // 否则使用原有逻辑
