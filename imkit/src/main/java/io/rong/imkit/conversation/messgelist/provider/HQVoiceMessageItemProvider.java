...
public class HQVoiceMessageItemProvider extends BaseMessageItemProvider<HQVoiceMessage> {
    ...
    protected void bindMessageContentViewHolder(
        ...
        if (uiMessage.getMessage().getMessageDirection() == Message.MessageDirection.SEND) {
            ...
            // RTL模式下翻转语音图标
            View sendIconView = holder.getView(R.id.rc_voice_send);
            ...
            if (message.getLocalPath() != null) {
                ...
                if (uiMessage.getState() == State.ERROR) {
                    ...
                    boolean listened = uiMessage.getMessage().getReceivedStatus().isListened();
                    boolean isConvertText =
                            message.getSttInfo() != null
                                    && message.getSttInfo().getStatus()
                                            == SpeechToTextInfo.SpeechToTextStatus.SUCCESS;
                    holder.setVisible(R.id.rc_voice_unread, !listened && !isConvertText);
                    holder.setVisible(R.id.rc_voice_download_error, false);
                    holder.setVisible(R.id.rc_download_progress, false);
                }
            }
        ...
        VoiceMessageItemProvider.applyVoiceRootLayoutForQuote(holder.itemView, hasQuoteCard);
