...
class UserInfoHelper {
    ...
    private List<RongUserInfoManager.UserDataObserver> mUserDataObservers;
    ...
    private void notifyUserChange(final UserInfo userInfo) {
        if (Thread.currentThread().equals(Looper.getMainLooper().getThread())) {
            for (RongUserInfoManager.UserDataObserver item : mUserDataObservers) {
    ...
    io.rong.imlib.model.Group getGroupInfo(final String groupId) {
     ...
     * IMCenter#init(Application, String, boolean)}之后调用{@link
     * RongUserInfoManager#setMessageAttachedUserInfo(boolean)}，
    ...
    void refreshGroupInfoCache(io.rong.imlib.model.Group groupInfo) {
        saveGroupInfoCache(groupInfo);
     ...
     */
    void addUserDataObserver(final RongUserInfoManager.UserDataObserver observer) {
        if (Thread.currentThread().equals(Looper.getMainLooper().getThread())) {
     ...
     */
    void removeUserDataObserver(final RongUserInfoManager.UserDataObserver observer) {
        if (Thread.currentThread().equals(Looper.getMainLooper().getThread())) {
