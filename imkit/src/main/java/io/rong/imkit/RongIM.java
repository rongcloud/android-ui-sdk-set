...
public class RongIM {
     ...
     * 连接服务器，在整个应用程序全局，只需要调用一次。
     *
     * @param option 连接配置 1. token 从服务端获取的用户身份令牌(Token) 2. timeLimit 连接超时时间，单位：秒。timeLimit <= 0，则 IM
     *     将一直连接，直到连接成功或者无法连接（如 token 非法） timeLimit > 0 ,则 IM 将最多连接 timeLimit 秒： 如果在 timeLimit
     *     秒内连接成功，后面再发生了网络变化或前后台切换，SDK 会自动重连； 如果在 timeLimit 秒无法连接成功则不再进行重连，通过 onError
     *     告知连接超时，您需要再自行调用 connect 接口.
    ...
    private static void connect(
            ConnectOption option, final RongIMClient.ConnectCallback connectCallback) {
    ...
    /**
     * 设置用户信息的提供者，供 RongIM 调用获取用户名称和头像信息。 设置后，当 sdk 界面展示用户信息时，会回调 {@link
    ...
    /**
     * 向本地会话中插入一条消息，方向为接收。这条消息只是插入本地会话，不会实际发送给服务器和对方。 插入消息需为入库消息，即 {@link
     ...
     * 加入聊天室。
     *
     * <p>如果聊天室不存在，sdk 会创建聊天室并加入，如果已存在，则直接加入
     *
     * <p>加入聊天室时，可以选择拉取聊天室消息数目。
     *
     * @param chatroomId 聊天室 Id。
     * @param defMessageCount 进入聊天室拉取消息数目，-1 时不拉取任何消息，0 时拉取 10 条消息，最多只能拉取 50
     *     条。（加入聊天室时会传本地最后一条消息的时间戳，拉取的是这个时间戳之后的消息。比如：这个时间戳之后有3条消息，defMessageCount传10，也只能拉到3条消息。）
     * @param callback 状态回调。
     */
    ...
    /**
     * 下载文件。和{@link #downloadMedia(Conversation.ConversationType, String, RongIMClient.MediaType,
     * String, RongIMClient.DownloadMediaCallback)}的区别是，该方法支持取消操作。
     *
     * <p>用来获取媒体原文件时调用。如果本地缓存中包含此文件，则从本地缓存中直接获取，否则将从服务器端下载。
     *
     * @param message 文件消息。
    ...
    public void getRemoteHistoryMessages(
            ...
            RongIMClient.ResultCallback<List<Message>> callback) {
        RongIMClient.getInstance()
                .getRemoteHistoryMessages(conversationType, targetId, dataTime, count, callback);
    }

    /**
     * 根据会话类型和目标 Id，拉取某时间戳之前或之后的 N 条远端历史消息记录。
     *
     * <p>该方法支持拉取指定时间之前或之后的远端历史消息
     *
     * @param conversationType 会话类型。不支持传入 ConversationType.CHATROOM。
     * @param targetId 目标 Id。根据不同的 conversationType，可能是用户 Id、讨论组 Id、群组 Id。
     ...
     * 发送多媒体消息
     *
     * <p>发送前构造 {@link Message} 消息实体，消息实体中的 content 必须为 {@link FileMessage}, 否则返回失败。
     *
     * @param message 发送消息的实体。
     * @param pushContent 当下发 push 消息时，在通知栏里会显示这个字段。 发送文件消息时，此字段必须填写，否则会收不到 push 推送。
     * @param pushData push 附加信息。如果设置该字段，用户在收到 push 消息时，能通过 {@link
    ...
    /**
     * 发送多媒体消息，可以使用该方法将多媒体文件上传到自己的服务器。 使用该方法在上传多媒体文件时，会回调 {@link
     * IRongCallback.ISendMediaMessageCallbackWithUploader#onAttached(Message,
     * IRongCallback.MediaMessageUploader)} 此回调中会携带 {@link IRongCallback.MediaMessageUploader}
     * 对象，使用者只需要调用此对象中的 {@link IRongCallback.MediaMessageUploader#update(int)} 更新进度 {@link
     * IRongCallback.MediaMessageUploader#success(Uri)} 更新成功状态，并告知上传成功后的文件地址 {@link
    ...
    /**
     * 设置当前用户信息。 如果开发者没有实现用户信息提供者，而是使用消息携带用户信息，需要使用这个方法设置当前用户的信息， 然后在{@link #init(Application,
     * String, boolean)}之后调用{@link
    ...
    public void setMessageReceivedStatus(
            ...
            final Conversation.ConversationType conversationType,
            final String targetId,
            final Message.ReceivedStatus receivedStatus,
            final RongIMClient.ResultCallback<Boolean> callback) {
        RongIMClient.getInstance()
