    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == RESULT_CODE && resultCode == Activity.RESULT_OK) {
            ...
            LocationMessage locationMessage =
                    LocationMessage.obtain(lat, lng, poi, Uri.parse(thumb));
            io.rong.imlib.model.Message message =
                    io.rong.imlib.model.Message.obtain(
                            mTargetId, mConversationType, locationMessage);
            IMCenter.getInstance()
                    .sendMessage(
                            message, null, null, null); // Todo check 是否需要调用 sendLocationMessage()
            if (mConversationType.equals(Conversation.ConversationType.PRIVATE)) {
                RongIMClient.getInstance()
                        .sendTypingStatus(mConversationType, mTargetId, "RC:LBSMsg");
            }
        }
    }
