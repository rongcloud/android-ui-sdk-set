...
public class StickerPackageDbTask {
    ...
    public synchronized static StickerPackageDbTask getInstance() {
        if (instance == null) {
    ...
    public void setPackageDownload(StickerPackage stickerPackage) {
        StickerPackageTable.setDownload(db, stickerPackage.getPackageId(), stickerPackage.isDownload());
    ...
    /**
     * 排序规则：先按是否预加载排序，预加载（preload=1）在前；
     * 再按order排序，order值小的在前
     */
    public List<StickerPackage> getStickerPackages() {
        ...
        for (StickerPackage stickerPackage : packages) {
            List<Sticker> stickers = StickerTable.getStickersByPackageId(
                    db, stickerPackage.getPackageId());
