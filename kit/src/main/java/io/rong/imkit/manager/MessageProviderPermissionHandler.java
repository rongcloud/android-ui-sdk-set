...
public class MessageProviderPermissionHandler {
    ...
    public boolean handleMessageClickPermission(UiMessage uiMessage, Fragment fragment) {
        ...
        if (PermissionCheckUtil.checkPermissions(fragment.getActivity(), permissions)) {
            ...
            PermissionCheckUtil.requestPermissions(fragment, permissions, REQUEST_CODE_ITEM_PROVIDER_PERMISSIONS);
            return true;
        }
    }

    public void addMessageContent(Class<? extends MessageContent> messageContentClass, String[] permissions) {
    ...
    public void onRequestPermissionsResult(Activity activity, String[] permissions, int[] grantResults) {
        ...
        BaseMessageItemProvider provider = (BaseMessageItemProvider) RongConfigCenter.conversationConfig().getMessageListProvider().getProvider(uiMessage);
        ((IMessageProviderPermissionHandler) provider).handleRequestPermissionsResult(activity, uiMessage, permissions, grantResults);
