...
public class MessageProcessor {
    ...
    public static void getMessagesDirection(final MessageViewModel messageViewModel, long sentTime, int count, final boolean isForward, final GetMessageCallback callback) {
        ...
        RongCoreClient.getInstance().getMessages(messageViewModel.getCurConversationType(), messageViewModel.getCurTargetId(), historyMessageOption, new IRongCoreCallback.IGetMessageCallback() {
            @Override
            public void onComplete(List<Message> messageList, IRongCoreEnum.CoreErrorCode errorCode) {
                IRongCoreEnum.ConversationLoadMessageType type = RongConfigCenter.conversationConfig().getConversationLoadMessageType();
                if (!isForward) {
                ...
                if (IRongCoreEnum.ConversationLoadMessageType.ASK.equals(type)) {
                    if (callback != null) {
                        messageViewModel.executePageEvent(new ShowLoadMessageDialogEvent(callback, messageList));
                        callback.onErrorAsk(messageList);
                    }
                } else if (IRongCoreEnum.ConversationLoadMessageType.ONLY_SUCCESS.equals(type)) {
    ...
    public static void getMessagesAll(final MessageViewModel messageViewModel, final long sentTime, int before, final int after, final GetMessageCallback callback) {
        ...
        RongCoreClient.getInstance().getMessages(messageViewModel.getCurConversationType(), messageViewModel.getCurTargetId(), historyMessageOption, new IRongCoreCallback.IGetMessageCallback() {
            ...
            public void onComplete(List<Message> messageList, IRongCoreEnum.CoreErrorCode errorCode) {
                ...
                if (errorCode == IRongCoreEnum.CoreErrorCode.SUCCESS) {
                    getMessagesDescend(sentTime, after, messageViewModel, allData, errorCode, callback);
                    return;
                }

                if (IRongCoreEnum.ConversationLoadMessageType.ASK.equals(type)) {
                    if (callback != null) {
                        messageViewModel.executePageEvent(new ShowLoadMessageDialogEvent(callback, allData));
    ...
    private static void getMessagesDescend(final long sentTime, int after, final MessageViewModel viewModel, final List<Message> allData, final IRongCoreEnum.CoreErrorCode code, final GetMessageCallback callback) {
        HistoryMessageOption historyMessageOption = new HistoryMessageOption();
        historyMessageOption.setDataTime(sentTime);
        historyMessageOption.setCount(after);
        historyMessageOption.setOrder(HistoryMessageOption.PullOrder.DESCEND);
        RongCoreClient.getInstance().getMessages(viewModel.getCurConversationType(), viewModel.getCurTargetId(), historyMessageOption, new IRongCoreCallback.IGetMessageCallback() {
    ...
    private static void initUnreadMessage(final MessageViewModel messageViewModel, final int unreadMessageCount) {
    ...
    private static void initMentionedMessage(Conversation conversation, final MessageViewModel messageViewModel) {
