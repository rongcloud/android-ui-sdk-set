...
public class NormalState implements IMessageState {
    ...
    public void init(final MessageViewModel messageViewModel, Bundle bundle) {
        MessageProcessor.getMessagesDirection(messageViewModel, 0, DEFAULT_COUNT, true, new MessageProcessor.GetMessageCallback() {
            @Override
    ...
    public void onLoadMore(final MessageViewModel viewModel) {
        MessageProcessor.getMessagesDirection(viewModel, viewModel.getLoadMoreSentTime() + 1, DEFAULT_COUNT, false, new MessageProcessor.GetMessageCallback() {
            @Override
            public void onSuccess(List<Message> list, boolean loadOnlyOnce) {
    ...
    public void onRefresh(final MessageViewModel viewModel) {
        if (!isRefreshLoading) {
            ...
            MessageProcessor.getMessagesDirection(viewModel, viewModel.getRefreshSentTime(), DEFAULT_COUNT, true, new MessageProcessor.GetMessageCallback() {
    ...
    private void getMentionMessage(final MessageViewModel viewModel,
        ...
        if (isNewMentionMessage) {
            if (!isLoading) {
                isLoading = true;
                MessageProcessor.getMessagesAll(viewModel, message.getSentTime(), DEFAULT_COUNT, DEFAULT_COUNT, new MessageProcessor.GetMessageCallback() {
                    @Override
                    public void onSuccess(List<Message> list, boolean loadOnlyOnce) {
            ...
            MessageProcessor.getMessagesDirection(viewModel, message.getSentTime() - 1, DEFAULT_COUNT, false, new MessageProcessor.GetMessageCallback() {
                @Override
                public void onSuccess(List<Message> list, boolean loadOnlyOnce) {
    ...
    public void onHistoryBarClick(final MessageViewModel messageViewModel) {
        ...
        if (firstUnreadMessage != null) {
            MessageProcessor.getMessagesDirection(messageViewModel, firstUnreadMessage.getSentTime() - 1, DEFAULT_COUNT - 1, false, new MessageProcessor.GetMessageCallback() {
                @Override
                public void onSuccess(List<Message> list, boolean loadOnlyOnce) {
    ...
    private void executeHistoryBarClick(List<Message> messageList, MessageViewModel messageViewModel) {
