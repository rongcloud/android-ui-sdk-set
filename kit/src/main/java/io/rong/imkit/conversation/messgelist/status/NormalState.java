...
/** /~chinese 会话页面当前状态，正常模式 */

/** /~english Current state of conversation page, normal mode */
public class NormalState implements IMessageState {
    ...
    /**
     * /~chinese 正常模式，初始化，拉取本地历史记录，处理未读数
     *
    ...
    /**
     * /~english Normal mode, initialize, pull local history, process unread number
     *
     * @param messageViewModel MessageViewModel
     * @param bundle Bundle
     */
    ...
    private void initMentionedMessage(
    ...
    /**
     * /~chiense 正常模式不需要上拉加载更多，直接关闭
     *
     * @param viewModel MessageViewModel
     */

    /**
     * /~english Normal mode, do not pull up and load more, just close it
     *
     * @param viewModel MessageViewModel
     */
    @Override
    ...
    /**
     * /~english Normal mode, process according to the procedures
     *
     * @param viewModel MessageViewModel
                                    ...
                                    public void onSuccess(List<Message> messages) {
                                        if (messages != null && messages.size() > 0) {
                                            ...
                                            if (messages.size() < DEFAULT_COUNT * 2 + 1) {
                                                ...
                                                viewModel.onLoadMoreMessage(
                                                        messages.subList(0, DEFAULT_COUNT * 2));
                                        ...
                                        isLoading = false;
                                        int position =
                                                viewModel.findPositionByMessageId(
        ...
        if (firstUnreadMessage != null) {
            RongIMClient.getInstance()
                    .getHistoryMessages(
                            messageViewModel.getCurConversationType(),
                            messageViewModel.getCurTargetId(),
                            firstUnreadMessage.getSentTime(),
                            0,
                            DEFAULT_COUNT - 1,
                            new RongIMClient.ResultCallback<List<Message>>() {
                                @Override
