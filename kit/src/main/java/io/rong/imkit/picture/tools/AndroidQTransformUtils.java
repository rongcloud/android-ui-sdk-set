...
public class AndroidQTransformUtils {
     ...
     * @return
     */
    public static String parseVideoPathToAndroidQ(
            Context ctx, String path, String fileName, String mineType) {
        try {
            String suffix = PictureMimeType.getLastImgSuffix(mineType);
            File filesDir =
                    ctx.getApplicationContext().getExternalFilesDir(Environment.DIRECTORY_MOVIES);
            if (filesDir != null) {
                String newPath =
                        new StringBuffer()
                                .append(filesDir)
     ...
     * @return
     */
    public static String parseImagePathToAndroidQ(
            Context ctx, String path, String fileName, String mineType) {
        try {
            String suffix = PictureMimeType.getLastImgSuffix(mineType);
     ...
     * @return
     */
    public static String copyImagePathToDirectoryPictures(
            Context ctx, String uri, String fileName, String mimeType) {
        try {
     ...
     * @return
     */
    public static String parseAudioPathToAndroidQ(
            Context ctx, String path, String fileName, String mineType) {
        try {
            String suffix = PictureMimeType.getLastImgSuffix(mineType);
            File filesDir =
                    ctx.getApplicationContext().getExternalFilesDir(Environment.DIRECTORY_MUSIC);
            if (filesDir != null) {
                String newPath =
                        new StringBuffer()
                                .append(filesDir)
                                ...
                                .append(
                                        TextUtils.isEmpty(fileName)
                                                ? DateUtils.getInstance().getCreateFileName("AUD_")
                                                        + suffix
                                                : fileName)
                                .toString();
