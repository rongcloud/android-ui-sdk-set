...
public class DbDataSource {
    ...
    void getUserInfo(final String userId, final Consumer<User> callback) {
        ExecutorHelper.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                User user = Option.ofObj(database).map(new Func1<UserDatabase, User>() {
                    @Override
    ...
    void getGroupInfo(final String groupId, final Consumer<Group> callback) {
        ExecutorHelper.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                Group group = Option.ofObj(database).map(new Func1<UserDatabase, Group>() {
                    @Override
                    public Group call(UserDatabase userDatabase) {
                        try {
                            return userDatabase.getGroupDao().getGroup(groupId);
                        } catch (Exception e) {
    ...
    void getGroupUserInfo(final String groupId, final String userId, final Consumer<GroupMember> callback) {
        ExecutorHelper.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                GroupMember groupMember = Option.ofObj(database).map(new Func1<UserDatabase, GroupMember>() {
                    @Override
                    public GroupMember call(UserDatabase userDatabase) {
                        try {
                            return userDatabase.getGroupMemberDao().getGroupMember(groupId, userId);
                        } catch (Exception e) {
                            RLog.e(TAG, "getGroupMember fail", e);
                            return null;
                        }

                    }
                }).orDefault(new Func0<GroupMember>() {
                    @Override
    ...
    void refreshUserInfo(@NonNull final User user) {
        ExecutorHelper.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                Option.ofObj(database).ifSome(new Action1<UserDatabase>() {
                    @Override
                ...
                Option.ofObj(database).ifSome(new Action1<UserDatabase>() {
                    ...
                    public void call(UserDatabase userDatabase) {
                        try {
                            ...
                            RLog.e(TAG, "insertGroupMember fail", e);
                        }
                    }
                }).ifNone(new Action0() {
                    @Override
    ...
    void getLimitUser(final int limit, final Consumer<List<User>> callback) {
        ExecutorHelper.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                List<User> user = Option.ofObj(database).map(new Func1<UserDatabase, List<User>>() {
                    @Override
                    public List<User> call(UserDatabase userDatabase) {
                        try {
                            return userDatabase.getUserDao().getLimitUsers(limit);
                        } catch (Exception e) {
                            RLog.e(TAG, "getUser fail", e);
                            return null;
                        }
    ...
    void getLimitGroup(final int limit, final Consumer<List<Group>> callback) {
        ExecutorHelper.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                List<Group> group = Option.ofObj(database).map(new Func1<UserDatabase, List<Group>>() {
                    @Override
                    public List<Group> call(UserDatabase userDatabase) {
                        try {
                            return userDatabase.getGroupDao().getLimitGroups(limit);
                        } catch (Exception e) {
                            RLog.e(TAG, "getUser fail", e);
                            return null;
                        }
                ...
                }).orDefault(new Func0<List<Group>>() {
                    @Override
    ...
    void getLimitGroupMember(final int limit, final Consumer<List<GroupMember>> callback) {
        ExecutorHelper.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                List<GroupMember> groupMembers = Option.ofObj(database).map(new Func1<UserDatabase, List<GroupMember>>() {
                    @Override
                    public List<GroupMember> call(UserDatabase userDatabase) {
                        try {
                            return userDatabase.getGroupMemberDao().getLimitGroupMembers(limit);
                        } catch (Exception e) {
                            RLog.e(TAG, "getUser fail", e);
                            return null;
                        }
                ...
                }).orDefault(new Func0<List<GroupMember>>() {
                    @Override
